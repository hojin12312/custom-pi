...
* 🕐 **`bg.ts` (Bash Background Runner)**: Long-running bash commands (downloads, builds) can be pushed to the background so the agent keeps working on other tasks.
  ...
  * **🆕 Direct backgrounding — Phase 1 (2026-08-20, [redesign plan](docs/BG-REDESIGN-PLAN.md))** — wrapper/tail-free path: the extension `spawn()`s a detached bash (own process group) directly, logging to `/tmp/pi-bg/<jobid>/log`. No `tail -f`, no 0.5s polling wrapper → no orphan tails, no pipe pollution, no per-command overhead for unmarked commands.
    * **`/bgrun <command>`** (user) — runs the command in the background and returns immediately; completion is auto-injected as above. Registered unconditionally (it creates jobs). Renamed to `/bg` in Phase 2 (today's `/bg [id]` = mid-run transition, being retired).
    ...
    * **Phase 1 is parallel deployment**: unmarked commands keep the legacy wrap+`ctrl+q` behavior exactly. The default switch (wrapper removal) is Phase 2, gated by the `PI_BG_OPT_IN=1` env var. Tests: `tests/bg-redesign.test.mjs` (PGID isolation, exit codes, multi-line cmds, auto-inject, quiet, no orphan tail).
